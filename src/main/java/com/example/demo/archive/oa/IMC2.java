...
public class IMC2 {
    ...
    public static List solution(List<Integer> arrival, List<Integer> street) {
        int n=arrival.size();
        ...
        int[] ans = new int[n];
        for (int i = 0; i < n; i++) time_car.add(new int[]{arrival.get(i), i});
        ...
        while (!time_car.isEmpty() || !main_street.isEmpty() || !first_avenue.isEmpty()) {
            ...
            while (!time_car.isEmpty() && time_car.peek()[0] == cur_time) {
                ...
                if (street.get(cur_time_car[1]) == 0) main_street.add(cur_time_car);
                else if (street.get(cur_time_car[1]) == 1) first_avenue.add(cur_time_car);
            }
        ...
        List<Integer> list11 =Arrays.stream(ans).boxed().collect(Collectors.toList());
        return list11;
    }
}
